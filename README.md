...
> [!NOTE]
> リアルタイムトランスコード視聴について
> 本Dockerイメージは軽量化のためデフォルトで`ffmpeg`を同梱していません．そのため，EDCB側のWebUIからの直接的なリアルタイムトランスコード再生機能は利用できません．
> リアルタイム視聴や録画番組のストリーミング再生には，本環境に同梱されている，KonomiTV(`https://192-168-x-x.local.konomi.tv:7000/`)のご利用を推奨します．
